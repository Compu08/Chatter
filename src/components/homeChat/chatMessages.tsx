...
function ChatMessages({ chatsData, chatId, setUserChatData }: ChatsMessagesProps) {
    ...
    useEffect(() => {
        ...
        //Ref para posicionamiento de la scroll al final
    ...
    return (
        <div className="chat-screen d-flex flex-column flex-grow-1 px-4">
            <div className="chat-inside-box d-flex flex-column">
                {chatMessages[0] ? chatMessages[0].messages.map((msg:any, i:any) => (
                    <div key={i} className={msg.received ? "contactMsg py-2 px-3 align-self-start my-3 d-flex gap-3" : "myMsg py-2 px-3 align-self-end my-3 d-flex gap-3"}>
                        <div>
                            {msg.message}
                        </div>
